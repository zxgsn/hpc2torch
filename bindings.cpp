...
void softmax_nv_f32(torch::Tensor input, torch::Tensor output, int size, int dimsize, int stride);
void attention_nv_f32(torch::Tensor Q, torch::Tensor K, torch::Tensor V, int N, int d, torch::Tensor output);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
      ...
      m.def("softmax", &softmax_nv_f32, "Cuda Core softmax function",
            py::arg("input"), py::arg("output"), py::arg("size"), py::arg("dimsize"), py::arg("stride"));

      m.def("attention", &attention_nv_f32, "Cuda Core attention function",
